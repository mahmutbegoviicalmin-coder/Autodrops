  ...
  return (
    <header className="glass-effect border-b border-gray-800/50 sticky top-0 z-40 backdrop-blur-sm">
      <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-6">
        ...
        <div className="flex justify-between items-center h-16 lg:h-20">
          ...
          <div className="flex items-center space-x-1 lg:space-x-2 flex-shrink-0">
            ...
            <button
              ...
              className="lg:hidden p-2 rounded-lg bg-gray-900/50 border border-gray-700/50 text-gray-300 hover:text-white hover:bg-gray-800/50 hover:border-purple-500/50 transition-colors"
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden mt-2 border border-gray-700/50 rounded-xl bg-gray-900/90 backdrop-blur-sm p-3 animate-fade-in">
            <div className="grid gap-2">
              <button
                onClick={() => { onGoHome?.(); setIsMobileMenuOpen(false); }}
                className="w-full text-left px-3 py-2 rounded-lg text-gray-300 hover:text-white hover:bg-gray-800/50 transition-colors"
              >
                Home
              </button>
              <button
                onClick={() => { onViewChange('products'); setIsMobileMenuOpen(false); }}
                className={`w-full text-left px-3 py-2 rounded-lg transition-colors ${selectedView === 'products' ? 'bg-gradient-to-r from-purple-600/20 to-blue-600/20 text-white' : 'text-gray-300 hover:text-white hover:bg-gray-800/50'}`}
              >
                Products
              </button>
              <button
                onClick={() => { onViewChange('orders'); setIsMobileMenuOpen(false); }}
                className={`w-full text-left px-3 py-2 rounded-lg transition-colors ${selectedView === 'orders' ? 'bg-gradient-to-r from-purple-600/20 to-blue-600/20 text-white' : 'text-gray-300 hover:text-white hover:bg-gray-800/50'}`}
              >
                Orders
              </button>
              {onToggleFilters && (
                <button
                  onClick={() => { onToggleFilters(); setIsMobileMenuOpen(false); }}
                  className="w-full text-left px-3 py-2 rounded-lg text-gray-300 hover:text-white hover:bg-gray-800/50 transition-colors"
                >
                  Toggle Filters
                </button>
              )}
              <a href="/privacy" className="px-3 py-2 rounded-lg text-gray-300 hover:text-white hover:bg-gray-800/50 transition-colors">Privacy</a>
              <a href="/terms" className="px-3 py-2 rounded-lg text-gray-300 hover:text-white hover:bg-gray-800/50 transition-colors">Terms</a>
            </div>
          </div>
        )}
      </div>
